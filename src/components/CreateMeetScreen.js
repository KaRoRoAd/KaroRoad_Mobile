...
const CreateMeetScreen = ({navigation, route}) => {
  ...
  const [endDate, setEndDate] = useState(
    route.params?.selectedDate 
      ? new Date(new Date(route.params.selectedDate).getTime() + 3600000)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        Alert.alert('Błąd', 'Nie udało się dodać spotkania');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Dodaj nowe spotkanie</Text>
